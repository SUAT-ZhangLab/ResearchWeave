# Install ResearchWeave

This is the installation guide for AI agents and people. Proceed when the user asks to install or use ResearchWeave.
...
2. Clone `https://github.com/SUAT-ZhangLab/ResearchWeave.git` into a new working directory, or download and extract the repository ZIP.
...
5. Run `python <installed-directory>/researchweave.py doctor`. Report the actual directory and result.
6. Explain how to reload skills or start a new session. Codex uses `$researchweave`; Claude Code uses `/researchweave`.
...
|---|---|---|
| Codex | `python install.py --agent codex` | `~/.agents/skills/researchweave` |
| Codex using the older skill directory | `python install.py --agent codex-legacy` | `$CODEX_HOME/skills/researchweave`, otherwise `~/.codex/skills/researchweave` |
| Claude Code (local) | `python install.py --agent claude` | `~/.claude/skills/researchweave` |
| Another agent with SKILL.md support | `python install.py --agent generic --destination "/actual/skill/path/researchweave"` | The specified directory |
...
The repository root is also compatible with skill installers that accept a repository path of `.` with the name `researchweave`.

## Use without installing a skill

Open this repository in the agent and ask it to read `AGENTS.md` and `SKILL.md`.
Run `python researchweave.py doctor` from its root. Use an explicitly selected research directory for each task.
...
```powershell
Get-Content config/era.Dockerfile -Raw | wsl.exe -d Ubuntu-22.04 -u root --cd / --exec sh -c 'systemctl start docker && docker build -t researchweave-runner:0.1.0 -'
$env:RESEARCHWEAVE_DOCKER_MODE = 'wsl'
$env:RESEARCHWEAVE_WSL_DISTRO = 'Ubuntu-22.04'
python researchweave.py doctor --container
```

`RESEARCHWEAVE_DOCKER_MODE` accepts `auto` (default), `native`, or `wsl`. Auto prefers a `docker` command on PATH;
otherwise Windows uses WSL. `RESEARCHWEAVE_WSL_DISTRO` selects the actual distribution.
...
Install ResearchWeave into its new skill directory. Existing study folders remain readable, including those created under the previous project name. The previous CLI filename is retained as a compatibility entry point; use `researchweave.py` for new commands.
