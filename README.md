...
| Source<br>来源 | What informed ResearchWeave<br>ResearchWeave 参考的内容 | What is included here<br>本项目收录的材料 |
|---|---|---|
| [Robin — FutureHouse](https://github.com/Future-House/robin) · [Ghareeb et al., 2026](https://doi.org/10.1038/s41586-026-10652-y)<br>Robin：FutureHouse | Organizing literature research, experimental ideas, candidate comparisons, data interpretation, and follow-up work<br>组织文献研究、实验构想、候选比较、数据解释和后续研究 | Attributed public prompts, selected source files, and adapted role guidance<br>注明来源的公开提示词、部分源代码及改写的角色指引 |
| Co-Scientist · [Gottweis et al., 2026, and supplementary information](https://doi.org/10.1038/s41586-026-10644-y)<br>Co-Scientist：论文与补充材料 | Generating hypotheses, reflecting on observations, comparing explanations, improving proposals, and synthesizing reviews<br>生成假设、反思观察、比较解释、改进方案和综合评议 | Public supplementary material, extracted templates, and adapted research methods<br>公开补充材料、提取的模板及改写的研究方法 |
| [ERA — Google Research](https://github.com/google-research/era) · [Aygün et al., 2026](https://doi.org/10.1038/s41586-026-10658-6)<br>ERA：Google Research | Improving candidate programs through execution feedback and search<br>根据执行反馈和搜索改进候选程序 | The upstream FUTS search implementation, public prompt/task extracts, and a project-written stepwise interface<br>原始 FUTS 搜索实现、公开提示词和任务摘录，以及本项目编写的分步接口 |
| [Finch — FutureHouse contributors, source snapshot](https://github.com/Future-House/finch/tree/aea66fdf2dd2be827727de50a73cae60dff59972)<br>Finch：FutureHouse 贡献者，指定版本源代码 | Additional publicly available data-analysis prompts associated with the broader source review<br>在整理来源时补充收集的公开数据分析提示词 | An attributed supplementary prompt collection<br>注明来源的补充提示词集 |
...
```mermaid
flowchart TD
    A["Research question and available materials<br/>研究问题与现有材料"] --> B["Read evidence and check the data<br/>阅读证据，检查数据"]
    B --> C["Form or refine testable explanations<br/>提出或改进可检验的解释"]
    C --> D["Choose comparisons and analysis methods<br/>选择比较方式和分析方法"]
    D --> E{"Does this question need computation?<br/>是否需要计算？"}
    E -->|No / 不需要| H["Interpret evidence and write the report<br/>解释证据，撰写报告"]
    E -->|One analysis / 单次分析| F["Execute and record the analysis<br/>执行并记录分析"]
    E -->|Compare implementations / 比较不同程序| G["Run the optional ERA program search<br/>运行可选的 ERA 程序搜索"]
    F --> H
    G --> H
    H --> I{"Is a specific next step useful?<br/>是否还有值得开展的具体工作？"}
    I -->|New evidence or an unresolved question / 新证据或待解决的问题| B
    I -->|Task answered / 任务已完成| J["Deliver the conclusion and saved records<br/>交付结论与保存的记录"]
```
...
| Capability<br>用途 | What is needed<br>所需条件 |
|---|---|
| Read the methods and prompts<br>阅读方法和提示词 | An agent that can read the repository<br>能够读取仓库文件的智能体 |
| Install the skill and keep research records<br>安装技能并保存研究记录 | Python 3.10+; Python 3.12 is recommended<br>Python 3.10 或更高版本；推荐 Python 3.12 |
| Literature retrieval and model reasoning<br>检索文献与模型推理 | The host agent's own tools and account<br>当前智能体自带的工具和账号 |
| Execute candidate programs<br>执行候选程序 | Docker with Linux containers and the project runner image<br>支持 Linux 容器的 Docker，以及本项目的运行镜像 |
| Large datasets, R, GPU, or file-heavy analyses<br>大数据集、R、GPU 或大量文件分析 | An environment selected for that task<br>针对该任务配置的环境 |
...
| Type<br>类型 | Purpose<br>用途 |
|---|---|
| `evidence`<br>证据 | A source-supported observation and its evidence category<br>有来源支持的观察及其证据类别 |
| `hypothesis`<br>假设 | An explanation, predictions, and links to supporting records<br>解释、预测及相关支持记录的链接 |
| `analysis`<br>分析 | Inputs, method, comparison, sample unit, and evaluation criteria<br>输入、方法、比较对象、样本单位和评估标准 |
| `code`<br>代码 | The actual program used<br>实际使用的程序 |
| `result`<br>结果 | An observed outcome linked to its analysis and code<br>关联到具体分析和代码的实际结果 |
| `review`<br>评议 | A reasoned assessment of explanations or findings<br>对解释或发现进行有依据的评估 |
| `update`<br>更新 | What changed and which specific steps remain useful<br>说明发生了什么变化，以及哪些具体后续工作仍有用 |
...
```text
ResearchWeave/
  README.md                 Project background, workflow, and quick start / 项目背景、流程与快速开始
  INSTALL.md                Installation instructions for agents and people / 面向智能体和使用者的安装说明
  SKILL.md                  Research instructions loaded by the host agent / 由当前智能体读取的科研指引
  AGENTS.md / CLAUDE.md      Repository entry points / 仓库入口说明
  install.py                Self-contained skill installer / 可独立使用的技能安装程序
  researchweave.py                Command-line entry point / 命令行入口
  references/               Roles, operations, and source attribution / 角色、操作与来源说明
  scripts/                  Records, ERA interface, Docker runner, and tests / 研究记录、ERA 接口、Docker 运行工具与测试
  agent-prompts/             Attributed original prompts and extraction records / 注明来源的原始提示词与提取记录
  vendor/                   Selected upstream source and licenses / 选用的原始项目源代码与许可证
  config/                   Source versions and Docker build definition / 来源版本与 Docker 构建配置
  examples/                 Input templates and synthetic examples / 输入模板与人工构造示例
```
...
- **Ghareeb, A. E., Chang, B., Mitchener, L., et al. (2026).** [A multi-agent system for automating scientific discovery](https://doi.org/10.1038/s41586-026-10652-y). *Nature*, **655**, 497–505. Robin source used here: [Future-House/robin, commit `4a5cce3`](https://github.com/Future-House/robin/tree/4a5cce310f3bc7663a67117db88af43b84733ffe).
  <br>中文题意：用于自动化科学发现的多智能体系统。本项目使用上述 Robin 指定提交版本的源代码。
...
- **Gottweis, J., Weng, W.-H., Daryin, A., et al. (2026).** [Accelerating scientific discovery with Co-Scientist](https://doi.org/10.1038/s41586-026-10644-y). *Nature*, **655**, 487–496. Includes the public supplementary methods and prompts; the [supplementary PDF](agent-prompts/co-scientist/source/41586_2026_10644_MOESM1_ESM.pdf) and [extraction record](agent-prompts/co-scientist/manifest.json) are included in this repository.
  <br>中文题意：借助 Co-Scientist 加速科学发现。本仓库收录了公开补充方法和提示词，并提供补充 PDF 与提取记录。
...
- **Aygün, E., Belyaeva, A., Comanici, G., et al. (2026).** [An AI system to help scientists write expert-level empirical software](https://doi.org/10.1038/s41586-026-10658-6). *Nature*, **654**, 909–916. ERA source used here: [google-research/era, commit `b836730`](https://github.com/google-research/era/tree/b836730b5c000526af95116b1d0e2c60c8cf0a10).
  <br>中文题意：帮助科学家编写专家水平实证研究软件的 AI 系统。本项目使用上述 ERA 指定提交版本的源代码。
...
- **FutureHouse contributors.** [Finch: an aviary-based data science agent based on Jupyter notebooks](https://github.com/Future-House/finch/tree/aea66fdf2dd2be827727de50a73cae60dff59972). Software source, commit `aea66fd`; source snapshot retrieved 15 September 2026. See the [local prompt manifest](agent-prompts/robin-components/manifest.json) for the exact files and extracted text used here.
  <br>中文说明：Finch 是基于 aviary 和 Jupyter notebooks 的数据科学智能体。源代码版本为 `aea66fd`，于 2026 年 9 月 15 日获取；实际使用的文件和提取文本见上述本地提示词清单。
...
- **Billingsley, M. M., Singh, N., Ravikumar, P., Zhang, R., June, C. H., and Mitchell, M. J. (2020).** [Ionizable Lipid Nanoparticle-Mediated mRNA Delivery for Human CAR T Cell Engineering](https://doi.org/10.1021/acs.nanolett.9b04246). *Nano Letters*, **20**(3), 1578–1589.
  <br>中文题意：通过可离子化脂质纳米颗粒递送 mRNA，用于人 CAR T 细胞工程。
...
- **Hafner, M., Niepel, M., Chung, M., and Sorger, P. K. (2016).** [Growth rate inhibition metrics correct for confounders in measuring sensitivity to cancer drugs](https://doi.org/10.1038/nmeth.3853). *Nature Methods*, **13**, 521–527.
  <br>中文题意：用生长速率抑制指标修正癌症药物敏感性测量中的混杂因素。
...
- **Colic, M., Wang, G., Zimmermann, M., et al. (2019).** [Identifying chemogenetic interactions from CRISPR screens with drugZ](https://doi.org/10.1186/s13073-019-0665-3). *Genome Medicine*, **11**, 52.
  <br>中文题意：使用 drugZ 从 CRISPR 筛选中识别化合物与基因之间的相互作用。
...
- **Kapoor, S., and Narayanan, A. (2023).** [Leakage and the reproducibility crisis in machine-learning-based science](https://doi.org/10.1016/j.patter.2023.100804). *Patterns*, **4**(9), 100804.
  <br>中文题意：基于机器学习的科学研究中的数据泄漏与可重复性危机。
